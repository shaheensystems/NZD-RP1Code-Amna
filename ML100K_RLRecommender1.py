...
def run_full_population():
    """Runs the RL recommender over every user in the MovieLens dataset (not a sample) and
    ...
    d = pd.read_csv("Movielens100k.csv", sep=',', engine='python')
    data = np.array(d)
    firscol = list(data[:, 4])  # userId column
    fit = {}
    ...
    for z in usersList:
        ...
        for x in range(0, data.shape[0]):
            if data[x][4] == z:
                it.append(int(data[x][0]))  # movieId
                r.append(data[x][5])  # rating
        fit[z] = {
            'User_Number': z,
            'Items': it,
            'Rating': r
    ...
    Dictobj_Users, Dictobj_Products, _ = ML100K_KMeansClustering.KMeans_Clusters(filepath)
    notPred = 0
    StartStates = []
    dicty = {}
    ...
    plt.savefig('Figure_StatesVisitedByUser_MovieLens.png', dpi=150)
    plt.close()
    ...
    plt.savefig('Figure_ReturnByUser_MovieLens.png', dpi=150)
    plt.close()
