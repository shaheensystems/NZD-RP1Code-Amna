...
import ML100K_KMeansClustering
import ML100K_StartState2
...
class TwoDGridWorld(gym.Env):
    ...
    def __init__(self, size,Biclust,u1I,Dict):
        ...
        self.StartState = ML100K_StartState2.computeJaccard(self.Dictobj, u1I,self.Biclust)#Compute start state using Jaccard
    ...
    for i_episode in range(num_episodes):
        ...
        while True:
            ...
            if (strt == 0):
                ...
                env.Recitems, sscount = ML100K_StartState2.computeGoal(itemset, env.GridPos[row][col],
            ...
            if (((new_state)!=prevState)):# If both prevstate and new_state are same then no reward
                ...
                reward = ML100K_StartState2.computeMyReward(env.GridPos[R][C], env.GridPos[row][col],env.Dictobj)
...
def extractPolicy(Q,env,u1I,u1R) :
    ...
    while not done:
        ...
        if (((new_state)!=prevState)):# If both prevstate and new_state are same then no reward
            R = prevState // env.size
            C = prevState % env.size
            reward = ML100K_StartState2.computeMyReward(env.GridPos[R][C], env.GridPos[row][col],
                                                 env.Dictobj)
